...
STD_RESIDUES = PRO_STD_RESIDUES | RNA_STD_RESIDUES | DNA_STD_RESIDUES | MASK_STD_RESIDUES

PRO_STD_RESIDUES_VALS_SET = set((PRO_STD_RESIDUES | MASK_STD_RESIDUES).values())

PRO_RES_IDX_TO_RESNAME = {val: key for key, val in (PRO_STD_RESIDUES | MASK_STD_RESIDUES).items()}
...
# If we just train on crystals, we don't need the other residues.
CRYS_ELEMS = {i: idx for idx, i in enumerate(get_all_elems())} 
...
PROT_STD_RESIDUES_THREE_TO_ONE = {v: k for k, v in PROT_STD_RESIDUES_ONE_TO_THREE.items()}
PRO_RES_IDX_TO_RESNAME_ONE = {
    val: PROT_STD_RESIDUES_THREE_TO_ONE[key] for key, val in PRO_STD_RESIDUES.items()
}
...
BACKBONE_ATOMS_NO_OXYGEN = ["CA", "C", "N"]

MASK_RESNAME = "MSK"
MASK_ONE_LETTER = "-"
MASK_TOKEN_IDX = STD_RESIDUES[MASK_RESNAME]
UNK_TOKEN_IDX = STD_RESIDUES["UNK"]
...
LMPNN_ELEMENT_LIST = [item.upper() for item in LMPNN_ELEMENT_LIST]
LMPNN_ELEMENT_DICT = dict(zip(LMPNN_ELEMENT_LIST, range(1, len(LMPNN_ELEMENT_LIST))))

LMPNN_RESTYPE_INT_TO_STR = {
    0: "A",
    1: "C",
    2: "D",
    3: "E",
...
LMPNN_RES_IDX_TO_OUR_RES_IDX = {
    lmpnn_idx: PRO_STD_RESIDUES[PROT_STD_RESIDUES_ONE_TO_THREE[lmpnn_res]]
    for lmpnn_idx, lmpnn_res in LMPNN_RESTYPE_INT_TO_STR.items()
}

OUR_RES_IDX_TO_LMPNN_RES_IDX = {v: k for k, v in LMPNN_RES_IDX_TO_OUR_RES_IDX.items()}

intersect_ele_lists = set(LMPNN_ELEMENT_LIST).intersection(set(get_all_elems()))
OUR_ELE_TO_LMPNN_MAP = {key: key for key in intersect_ele_lists}
OUR_ELE_TO_LMPNN_MAP.update({"OG": "UUO", "MC": "UUP", "TS": "UUS", "NH": "UUT", "MO": "MB"})

ATOMIC_NUM_TO_ELE_NAME = {idx: ele_name for idx, ele_name in enumerate(get_all_elems())}
...
COORDINATION_DONORS = {
    6,             # carbenic C
    7, 8,          # N, O
    15,16,34,52,   # P, S, Se, Te
    9,17,35,53     # F, Cl, Br, I
}

COORDINATION_ACCEPTORS = (
    {5,13,14,32,50,82} |                 # B, Al, Si, Ge, Sn, Pb
    set(range(21,31)) | set(range(39,49)) |
    set(range(57,  83))                  # lanthanides + 3rd-row d-block
)


CCDC_TO_BIOTITE_BOND_ORDER = {
    0: 1,  # "unknown" → single (safest default)
    1: 1,  # single
    2: 2,  # double
    3: 3,  # triple
    4: 4,  # quadruple (rare)
    5: 9,  # aromatic
    # any other value (e.g. aromatic -> 5 to 9 is there, but delocalised -> 7, pi -> 9) → treat as *1*;
}