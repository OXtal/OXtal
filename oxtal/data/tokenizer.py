...
from oxtal.data.constants import ELEMS, CRYS_ELEMS, STD_RESIDUES
...
class TokenArray:
    ...
    def __getitem__(self, index):
        ...
        elif torch.is_tensor(index) and index.ndim == 0:
    ...
    def set_annotation(self, category, values):
        assert len(values) == len(self.tokens), "Length of values must match the number of tokens"
...
class AtomArrayTokenizer:
    ...
    def __init__(self, atom_array: AtomArray, crystal_only = False):
        self.atom_array = atom_array
        self.crystal_only = crystal_only
    ...
    def tokenize(self) -> list[Token]:
        ...
        for res in struc.residue_iter(self.atom_array):
            ...
            if res_token is not None and mol_type != "ligand":
                ...
                atom_indices = [i for i in range(total_atom_num, total_atom_num + atom_num)]
                ...
                for atom in res:
                    ...
                    if not self.crystal_only:
                        atom_token = ELEMS.get(atom_elem, None)
                    else:
                        # This ignores all the amino acids in tokenizer
                        atom_token = CRYS_ELEMS.get(atom_elem, None)
                    ...
                    token.atom_names = [self.atom_array[token.atom_indices[0]].atom_name]
                    tokens.append(token)
                    total_atom_num += 1
