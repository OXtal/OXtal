...
from oxtal.data.data_pipeline import DataPipeline
from oxtal.data.featurizer import Featurizer
from oxtal.data.tokenizer import TokenArray
from oxtal.data.utils import data_type_transform, make_dummy_feature
from oxtal.utils.cropping import CropData
from oxtal.utils.file_io import read_indices_csv
from oxtal.utils.logger import get_logger
from oxtal.utils.np_utils import run_lengths
...
class BaseSingleDataset(Dataset):
    ...
    return a dict of features and labels, the keys and the shape are defined in oxtal.data.utils
    """
    ...
    def __init__(
        ...
        msa_featurizer: Optional[Any] = None,
        ...
        **kwargs,
        ...
        self.reassign_continuous_chain_ids = kwargs.get("reassign_continuous_chain_ids", False)
        ...
        self.backbone_only = kwargs.get("backbone_only", False)
        self.monomer_only = kwargs.get("monomer_only", False)
        self.monomer_chain_selection_strategy = kwargs.get(
            "monomer_chain_selection_strategy", "longest"
        )
        ...
        self.limits = kwargs.get("limits", -1)  # Limit number of indices rows, mainly for test
        ...
        self.task_manager_configs = kwargs.get("task_manager", None)
        self.task_manager = hydra.utils.instantiate(self.task_manager_configs)
    ...
    def read_pdb_list(pdb_list: Union[list, str]) -> Optional[list]:
        ...
        if isinstance(pdb_list, list) or isinstance(pdb_list, ListConfig):
            return pdb_list
    ...
    def read_indices_list(self, indices_fpath: Union[str, Path]) -> pd.DataFrame:
        ...
        if self.max_n_token > 0:
            ...
            logger.info(f"[filtered by n_token ({self.max_n_token})] #Rows: {len(indices_list)}")
        ...
        for col_name, exclusion_list in self.exclusion_dict.items():
            ...
            logger.info(f"[Excluded by {col_name} -- {exclusion_list}] #Rows: {len(indices_list)}")
        ...
        if self.find_eval_chain_interface:
            ...
            if self.group_by_pdb_id:
                indices_list = [
                    ...
                    if len(
                        set(df["eval_type"].to_list()).intersection(set(EvaluationChainInterface))
                    )
                    > 0
                ]
            else:
                indices_list = indices_list[
                    indices_list["eval_type"].apply(lambda x: x in EvaluationChainInterface)
                ]
        if self.limits > 0 and len(indices_list) > self.limits:
            logger.info(f"Limit indices list size from {len(indices_list)} to {self.limits}")
    ...
    def __getitem__(self, idx: int):
        ...
        for _ in range(10):
            try:
                ...
                if data is None:  # for weird pdb with no protein
                    continue
                return data
    ...
    def _get_bioassembly_data(self, idx: int) -> tuple[list[dict[str, Any]], dict[str, Any]]:
        sample_indice = self._get_sample_indice(idx=idx)
    ...
    def process_bioassembly(self, sample_indice, bioassembly_dict, bioassembly_dict_fpath):
        token_centre_atom_indices = bioassembly_dict["token_array"].get_annotation(
            "centre_atom_index"
        )
        token_chain_id = bioassembly_dict["atom_array"][token_centre_atom_indices].chain_id
        ...
        if self.monomer_only:
            all_chains = np.unique(token_chain_id)
            chain_lengths = run_lengths(token_chain_id)

            seld_chain_id = None
            match self.monomer_chain_selection_strategy:
                case "longest":
                    seld_chain_id = np.argmax(chain_lengths)

                case "proportional":
                    probs = chain_lengths / chain_lengths.sum()
                    seld_chain_id = np.random.choice(np.arange(len(chain_lengths)), p=probs)

                case _:
                    raise ValueError(
                        "Had invalid monomer chain selection strategy"
                        + f"{self.monomer_chain_selection_strategy}"
                    )

            seld_chain = all_chains[seld_chain_id]
            seld_tokens = np.flatnonzero(token_chain_id == seld_chain)
            ...
            (
                ...
                token_array=bioassembly_dict["token_array"],
                atom_array=bioassembly_dict["atom_array"],
                selected_token_indices=seld_tokens,
            )
        ...
        if self.backbone_only:
            (
                bioassembly_dict["token_array"],
                bioassembly_dict["atom_array"],
            ) = CropData.remove_non_backbone_atoms(
    ...
    def process_one(self, idx: int, return_atom_token_array: bool = True) -> dict[str, dict]:
        """
        ...
        sample_indice, bioassembly_dict, bioassembly_dict_fpath = self._get_bioassembly_data(
            idx=idx
        )

        print("Running PDB:" + str(bioassembly_dict["pdb_id"]))

        task, t = self.task_manager.sample_task_and_time()
        print(f"Task: {task}, t: {t}")  # logging task and time

        # Masks out side chains randomly based on t. Also, clean_cano_seq_resname is added
        # in task manager, so if we're in eval add it separately below as later code needs it
        if self.is_train:
            masking_result = self.task_manager.mask(bioassembly_dict["atom_array"], task=task, t=t)

            bioassembly_dict.update(asdict(masking_result))
        else:
            bioassembly_dict["atom_array"].set_annotation(
                "clean_cano_seq_resname", np.copy(bioassembly_dict["atom_array"].cano_seq_resname)
            )

        pdb_id = (
            bioassembly_dict["pdb_id"] if not self.is_distillation else sample_indice["pdb_id"]
        )
        ...
        try:
            ...
            print(f"Had following error with PDB {pdb_id}: {e}")
            raise e
        ...
        asymn_id_to_chain_id = {atom.asym_id_int: atom.chain_id for atom in cropped_atom_array}
        chain_id_list = [
        ...
        # add true info in label dict
        true_restype_id_label = feat["true_restype_id"]
        UNK_ID = len(STD_RESIDUES)
        true_restype = [
            STD_RESIDUES[x] if x in STD_RESIDUES else UNK_ID for x in true_restype_id_label
        ]  # @NOTE: non aa/na are lumped as UNK_ID and not removed
        true_restype = [x for x in true_restype if x != -1]
        true_restype = torch.tensor(true_restype)

        # true prot_res type
        feat["true_prot_restype"] = torch.tensor(
            [PRO_STD_RESIDUES[x] for x in true_restype_id_label if x in PRO_STD_RESIDUES]
        )

        masked_residues = PRO_STD_RESIDUES | MASK_STD_RESIDUES
        cond = lambda x, i: (
            (x in masked_residues)
            and (true_restype_id_label[i] in masked_residues)
            and (cropped_token_array[i].value in PRO_STD_RESIDUES_VALS_SET)
        )
        ...
        feat["masked_prot_restype"] = torch.tensor(
            [
                masked_residues[x]
                for i, x in enumerate(feat["restype_id"])  # @TODO: check restype_id is masked
                if cond(x, i)
            ]
        )

        # mask
        assert len(cropped_token_array) == len(true_restype_id_label)
        prot_residue_mask = [
            1 if token.value in PRO_STD_RESIDUES_VALS_SET else 0 for token in cropped_token_array
        ]
        feat["prot_residue_mask"] = np.array(prot_residue_mask)

        # If there's no protein residues (even if there's protein atoms which
        # are part of standalone ligand AAs) then raise a warning, as typically
        # things will break in the model if there's no protein backbone residues.
        #
        # Since the mask is an int check that sum is equal to 0.
        # if feat["prot_residue_mask"].sum() <= 1:
        #     logger.warning(
        #         f"Had crop for PDB {bioassembly_dict['pdb_id']} "
        #         + "with 0 or 1 protein backbone residues"
        #     )

        feat["true_restype"] = true_restype
        label["true_restype"] = true_restype
        label_full["true_restype"] = true_restype

        feat["pdb_id"] = bioassembly_dict["pdb_id"]
        data = {
        ...
        # @FIXME: cano res is not correct!! need to fix add_cano_seq_resname, currently temp fix by skipping the batch
        return data
    ...
    def _get_eval_chain_interface_mask(
        ...
        if len(df) < 1:
            raise ValueError(f"Cannot find a chain/interface for evaluation in the PDB.")
        ...
        if len(df) < 1:
            raise ValueError(f"Cannot find a chain/interface for evaluation in the atom_array.")
    ...
    def get_feature_and_label(
        ...
        @TODO: Add masking to featurizer
        """
        ...
        if self.find_pocket:
            ...
            if self.find_all_pockets:
                ...
                all_lig_asym_ids = set(
                    full_atom_array[full_atom_array.label_entity_id == lig_entity_id].label_asym_id
                )
        ...
        # DR: added featurized sequence to label_dict
        labels_dict["msa"] = features_dict["msa"]
...
class WeightedMultiDataset(Dataset):
    ...
    def init_datasets(self):
        ...
        for dataset_index, (
            ...
            dataset_weight,
        ) in enumerate(zip(self.datasets, self.datapoint_weights, self.dataset_sample_weights)):
    ...
    def __getitem__(self, index: int) -> dict[str, dict]:
        return self.datasets[self.dataset_indices[index]][self.within_dataset_indices[index]]
...
def get_weighted_pdb_weight(
    ...
    weight = (
        beta
        * sum([alpha * chain_count[data_mode] for data_mode, alpha in alpha_dict.items()])
        / (cluster_size + eps)
    )
    return weight
...
def get_datasets(
    ...
    def _get_dataset_param(config_dict, dataset_name: str, stage: str):
        ...
        return {
            ...
            **config_dict["base_info"],
            ...
            "error_dir": error_dir,
            "msa_featurizer": None, #get_msa_featurizer(configs, dataset_name, stage),
            ...
            "use_reference_chains_only": config_dict.get("use_reference_chains_only", False),
            "backbone_only": config_dict.get("backbone_only", False),
            "monomer_only": config_dict.get("monomer_only", False),
            "monomer_chain_selection_strategy": config_dict.get(
                "monomer_chain_selection_strategy", "longest"
            ),
        }
    ...
    ), "Number of train sets should be equal to number of train sample weights"
    train_datasets = []
    datapoint_weights = []
    for train_name in data_config.train_sets:
        config_dict = dict(data_config[train_name])
        dataset_param = _get_dataset_param(config_dict, dataset_name=train_name, stage="train")
        dataset_param["ref_pos_augment"] = data_config.get("train_ref_pos_augment", True)
        dataset_param["limits"] = data_config.get("limits", -1)
        # add task manager configs
        dataset_param["task_manager"] = configs.get("task_manager", None)
        dataset_param["is_train"] = True
    ...
    for test_name in test_sets:
        config_dict = OmegaConf.to_container(data_config[test_name], resolve=True)
        dataset_param = _get_dataset_param(config_dict, dataset_name=test_name, stage="test")
        dataset_param["ref_pos_augment"] = data_config.get("test_ref_pos_augment", True)
        # add task manager configs
        dataset_param["task_manager"] = configs.get("task_manager", None)
        dataset_param["is_train"] = False
