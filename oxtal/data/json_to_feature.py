...
from oxtal.data.json_parser import (
...
class SampleDictToFeatures:
    def __init__(self, single_sample_dict, task_manager):
        ...
        for bond_info_dict in self.input_dict["covalent_bonds"]:
            ...
            for i in ["left", "right"]:
                ...
                if isinstance(atom_name, int):
                    ...
                    entity_dict = self.input_dict["sequences"][int(entity_id - 1)].values()
    ...
    def add_atom_array_attributes(
        atom_array: AtomArray, entity_poly_type: dict[str, str], bb_only: bool = False
    ) -> AtomArray:
        """
        ...
        Args:
            ...
            bb_only (bool): A flag which specifies if we're doing backbone-only training
                            or generation. Is only used when making the distogram
    ...
    def get_atom_array(
        self, atom_array: Optional[AtomArray] = None, bb_only: bool = False
    ) -> AtomArray:
        """
        Create a Biotite AtomArray and add attributes from the input dict.

        Args:
            atom_array (Optional[AtomArray]): If specified, don't rebuild the full
                                              atom array and instead only recreate
                                              the atom array attributes.
        ...
        atom_array = self.add_atom_array_attributes(atom_array, self.entity_poly_type, bb_only)
        return atom_array

    def get_feature_dict(
        self, atom_array=None, called_during_generation: bool = False, bb_only: bool = False, crystal_only: bool = False,
    ) -> tuple[dict[str, torch.Tensor], AtomArray, TokenArray]:
        """
        Generates a feature dictionary from the input sample dictionary.
        Crystal_only is whether we're only training on crystal data, this way we drop amino acid tokens, only keep elements
        ...
        Returns:
            A tuple containing:
                ...
                - A TokenArray object.
        """
        atom_array = self.get_atom_array(atom_array, bb_only=bb_only)

        aa_tokenizer = AtomArrayTokenizer(atom_array, crystal_only=crystal_only)
        token_array = aa_tokenizer.get_token_array()

        if not called_during_generation:
            masking_result = self.task_manager.mask(
                atom_array, self.task_manager.inference_task, self.task_manager.inference_time
            )

            atom_array, token_array = masking_result.atom_array, masking_result.token_array
