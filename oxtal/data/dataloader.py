...
from oxtal.data.dataset import Dataset, get_datasets
...
class WeightedSampler(Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        for i in range(self.i, len(indices)):
            self.i += 1
            yield indices[i]
        self.i = 0
...
class KeySumBalancedSampler(Sampler):
    ...
    def get_balanced_assignments(self):
        ...
        if isinstance(self.dataset.indices_list, list):
            ...
            dataset_values = [x[self.key].astype(int)[0] for x in self.dataset.indices_list]
        else:
        ...
        if len(all_indices) > total_samples:
            ...
            #                if i >= len(all_indices) - total_samples:
            #                    # Assign some previous sample that was already assigned
            #                    # which can be filtered out later
            #                    j = i
            #                else:
            #                    j = total_samples + i
            #                worker_assignments[i % self.num_replicas].append(all_indices[j])

            for i in range(len(all_indices) - total_samples):
                worker_assignments[i % self.num_replicas].append(all_indices[total_samples + i])
...
class IterDataLoader(StatefulDataLoader):
    """
    ...
    def __iter__(self):
        ...
        self.counter += 1
        _iterator = super().__iter__()
        return _iterator

    def state_dict(self):
        return {"counter": self.counter, **super().state_dict()}
...
def get_dataloaders(
    ...
    logger.info(f"train data size: {len(train_dataset)}, test size: {test_dataset_sizes}")
