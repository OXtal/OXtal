...
from oxtal.data.constants import STD_RESIDUES
from oxtal.model.modules.primitives import (
    AdaptiveLayerNorm,
    Attention,
    BiasInitLinear,
    LinearNoBias,
...
class DiffusionTransformer(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(n_blocks):
            block = DiffusionTransformerBlock(n_heads=n_heads, c_a=c_a, c_s=c_s, c_z=c_z)
    ...
    def forward(
        ...
        a, s, z = checkpoint_blocks(blocks, args=(a, s, z), blocks_per_ckpt=blocks_per_ckpt)
        del s, z
        return a
...
class AtomAttentionEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.linear_no_bias_d = LinearNoBias(in_features=3, out_features=self.c_atompair)
        self.linear_no_bias_invd = LinearNoBias(in_features=1, out_features=self.c_atompair)
        self.linear_no_bias_v = LinearNoBias(in_features=1, out_features=self.c_atompair)

        if self.has_coords:
            # Line9
            self.layernorm_s = LayerNorm(self.c_s)
            self.linear_no_bias_s = LinearNoBias(in_features=self.c_s, out_features=self.c_atom)
            # Line10
        ...
        self.linear_no_bias_q = LinearNoBias(in_features=self.c_atom, out_features=self.c_token)
    ...
    def forward(
        ...
        q_trunked_list, k_trunked_list, pad_info = rearrange_qk_to_dense_trunk(
            q=[input_feature_dict["ref_pos"].to(ddv), input_feature_dict["ref_space_uid"].to(ddv)],
            k=[input_feature_dict["ref_pos"].to(ddv), input_feature_dict["ref_space_uid"].to(ddv)],
            dim_q=[-2, -1],
            dim_k=[-2, -1],
            n_queries=self.n_queries,
            n_keys=self.n_keys,
        ...
        p_lm = (self.linear_no_bias_d(d_lm) * v_lm) * pad_info["mask_trunked"].unsqueeze(
            dim=-1
        )  # [..., n_blocks, n_queries, n_keys, C_atompair]
        ...
        if inplace_safe:
            p_lm += (
                self.linear_no_bias_invd(1 / (1 + (d_lm**2).sum(dim=-1, keepdim=True))) * v_lm
            )
        ...
        if r_l is not None:
            ...
            c_l = c_l.unsqueeze(dim=-3) + self.linear_no_bias_s(
                self.layernorm_s(
                    broadcast_token_to_atom(x_token=s, atom_to_token_idx=atom_to_token_idx)
                )
            )  # [..., N_sample, N_atom, c_atom]
        ...
        a = aggregate_atom_to_token(
            ...
            n_token=n_token,
            reduce="mean",  # NOTE: This could maybe be changed to sum?
        )  # [..., (N_sample), N_token, c_token]
...
class AtomAttentionDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.linear_no_bias_out = LinearNoBias(in_features=c_atom, out_features=n_out_ftrs)
        self.atom_transformer = AtomTransformer(
            n_blocks=n_blocks,
            n_heads=n_heads,
            c_atom=c_atom,
    ...
    def forward(
        ...
        if self.return_token_dim_tnsr:
            prot_res_mask = input_feature_dict["prot_residue_mask"].astype(bool)
            if r.ndim == 3:
                r = r[:, prot_res_mask]
            else:
                r = r[prot_res_mask]
