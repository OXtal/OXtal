...
from oxtal.model.modules.embedders import FourierEmbedding, RelativePositionEncoding
from oxtal.model.modules.primitives import LinearNoBias, Transition
from oxtal.model.modules.transformer import (
...
class DiffusionConditioning(nn.Module):
    ...
    def __init__(
        ...
        self.linear_no_bias_z = LinearNoBias(in_features=2 * self.c_z, out_features=self.c_z)
        # Line3-Line5:
    ...
    def forward(
        ...
        t_hat_noise_level_seq: Optional[torch.Tensor] = None,
        ...
        # Pair conditioning
        pair_z = torch.cat(tensors=[z_trunk, relpe], dim=-1)  # [..., N_tokens, N_tokens, 2*c_z]
        ...
        fourier_struct = self.fourier_embedding if not self.do_fourier_embed_seq else self.fourier_embedding_struct
        linear_struct = self.linear_no_bias_n if not self.do_fourier_embed_seq else self.linear_no_bias_n_struct
        layernorm_struct = self.layernorm_n if not self.do_fourier_embed_seq else self.layernorm_n_struct

        struct_noise_n = fourier_struct(
            t_hat_noise_level=torch.log(input=t_hat_noise_level_struct / self.sigma_data) / 4
        ).to(
            single_s.dtype
        )  # [..., N_sample, c_in]
        ...
        if self.do_fourier_embed_seq and t_hat_noise_level_seq is not None:
            seq_noise_n = self.fourier_embedding_seq(
                t_hat_noise_level=torch.log(input=t_hat_noise_level_seq / self.sigma_data) / 4
            ).to(
                single_s.dtype
            )  # [..., N_sample, c_in]

            seq_s = self.linear_no_bias_n_seq(self.layernorm_n_seq(seq_noise_n)).unsqueeze(
                dim=-2
            )  # [..., N_sample, N_tokens, c_s]
...
class DiffusionSchedule:
    ...
    def get_inference_noise_schedule(self) -> torch.Tensor:
        ...
        inference_noise_schedule = (
            ...
            * (
                self.s_max ** (1 / self.p)
                + time_step_lists * (self.s_min ** (1 / self.p) - self.s_max ** (1 / self.p))
            )
            ** self.p
        )
...
class DiffusionModule(nn.Module):
    ...
    def get_all_latents(
        ...
        reps_have_batch_dim = z_trunk.ndim == 4 and s_trunk.ndim == 3
        if not reps_have_batch_dim:
            s_trunk = expand_at_dim(s_trunk, dim=-3, n=N_sample)  # [..., N_sample, N_token, c_s]
            z_pair = expand_at_dim(
                z_pair, dim=-4, n=N_sample
            )  # [..., N_sample, N_token, N_token, c_z]

        if s_skip is not None and z_skip is not None:
            s_trunk = s_trunk + self.linear_no_bias_s_skip(self.layernorm_s_skip(s_skip))
    ...
    def forward(
        ...
        r_noisy = (
            x_noisy
            / torch.sqrt(self.sigma_data**2 + t_hat_noise_level_struct**2)[..., None, None]
        )
        ...
        s_ratio = (t_hat_noise_level_struct / self.sigma_data)[..., None, None].to(r_update.dtype)
        x_denoised = (
            1 / (1 + s_ratio**2) * x_noisy
            + t_hat_noise_level_struct[..., None, None] / torch.sqrt(1 + s_ratio**2) * r_update
        ).to(r_update.dtype)
...
class JointDiffusionModule(DiffusionModule):
    ...
    def forward(
        ...
        inplace_safe: bool = False,
        chunk_size: Optional[int] = None,
        compute_seq_logits: Optional[bool] = True,
    ) -> torch.Tensor:
        """One step denoise: x_noisy, noise_level -> x_denoised
