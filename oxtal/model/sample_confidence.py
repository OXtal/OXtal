...
from oxtal.metrics.clash import Clash
from oxtal.utils.distributed import traverse_and_aggregate
...
def _compute_full_data_and_summary(
    ...
    summary_confidence["gpde"] = (full_data["token_pair_pde"] * full_data["contact_probs"]).sum(
        dim=[-1, -2]
    ) / full_data["contact_probs"].sum(dim=[-1, -2])
    ...
    summary_confidence.update(
        calculate_chain_based_plddt(full_data["atom_plddt"], token_asym_id, atom_to_token_idx)
    )
    del pae_prob
    ...
    summary_confidence["num_recycles"] = torch.tensor(N_recycle, device=atom_coordinate.device)
    # TODO: disorder
...
def calculate_ptm(
    ...
    per_bin_weight = (1 / (1 + (bin_center / ptm_norm) ** 2)).to(pae_prob.device)  # [N_bins]
...
def calculate_chain_based_ptm(
    ...
    # Chain iptm
    chain_has_frame = [(asym_id_to_asym_mask[i] * has_frame).any() for i in range(N_chain)]
...
def calculate_chain_based_plddt(
    ...
    # Chain_pair_plddt
    chain_pair_plddt = torch.zeros(size=batch_shape + (N_chain, N_chain)).to(atom_plddt.device)
